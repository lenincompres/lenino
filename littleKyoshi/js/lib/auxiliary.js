...
Array.prototype.indexed = function (propOrArr) { //returns object with items indexed according a Prop or Array
  var obj = {};
  this.forEach((val, i) => Array.isArray(propOrArr) ? obj[propOrArr[i % propOrArr.length]] = val : obj[val[propOrArr]] = val);
  return obj;
}
/* ----------------------------CREATE DOM ---------------------------------- */
...
var createDOM = (foo, bar, atElem) => { // creates dom elements from a js obj or json/uri; supports p5.
  if (foo === null || foo === undefined || ['_tag', '_id'].includes(bar)) return;
  ...
  if (atElem && bar) { // event handlers: createDOM(handler, event, elem)
    if (['onready', 'onReady', 'onelement'].includes(bar)) // passes element to function
      return foo(atElem);
  ...
  const TAGGED = tag && tag.match(/^h[1-9]$/) || ['main', 'a', 'abbr', 'acronym', 'address', 'applet', 'area', 'article', 'aside', 'audio', 'b', 'base', 'basefont', 'bdo', 'big', 'blockquote', 'body', 'br', 'button', 'canvas', 'caption', 'center', 'cite', 'code', 'col', 'colgroup', 'datalist', 'dd', 'del', 'dfn', 'div', 'dl', 'dt', 'em', 'embed', 'fieldset', 'figcaption', 'figure', 'font', 'footer', 'form', 'frame', 'frameset', 'head', 'header', 'hr', 'html', 'i', 'iframe', 'img', 'input', 'ins', 'kbd', 'label', 'legend', 'li', 'link', 'map', 'mark', 'meta', 'meter', 'nav', 'noscript', 'object', 'ol', 'optgroup', 'option', 'p', 'param', 'pre', 'progress', 'q', 's', 'samp', 'script', 'section', 'select', 'small', 'source', 'span', 'strike', 'strong', 'style', 'sub', 'sup', 'table', 'tbody', 'td', 'textarea', 'tfoot', 'th', 'thead', 'time', 'title', 'tr', 'u', 'ul', 'var', 'video', 'wbr'].includes(tag);
